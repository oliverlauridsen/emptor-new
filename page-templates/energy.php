<?php 
/* Template Name: Energy */ 

?>

<?php wp_head();?>
<meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1">

<body>
	<div id="content">
		<section class="product_section_1" id="energy">
			<div class="product_black_header">
			</div>
		</section>
		<section class="product_section_2">
			<div class="product_page_close">
				<a href="trading#section_2">
					<img class="cross" src="<?php echo get_stylesheet_directory_uri(); ?>/img/cancel-music.png" alt="Close ballot box icon"> </a>
				<a href="trading#section_2">
					<img class="blue-cross" src="<?php echo get_stylesheet_directory_uri(); ?>/img/blue-cancel-music.png" alt="Close ballot box icon"> </a>
			</div>
			<h1> ENERGY </h1>

			<div class="ballot_boxes_text_wrapper">
				<div class="left_product_page_tablet_divider">
				<br>
                    Emptor International provides a tailor-made solution on the supply of the following products:
					<br><br>
				</div>
				<div class="right_product_page_tablet_divider">
                    <ul class="trading-product-list">

                        <li> Cables and wires, insulators & conductors </li>
                        <li> Switchgear and panels </li>
                        <li> Distribution boards </li>
                        <li> Transformers and diesel generators </li>
                        <li> Energy meters </li>
                        <li> Solar systems </li>
                        <li> Lightning fixtures </li>
                    </ul>				</div>
			</div>
		</section>
		<div class="product_footer">
		<a href="construction">
				<div class="arrow_right arrow_trading">
					<strong>Next</strong>
					<i class="fa fa-arrow-right" aria-hidden="true"></i>
				</div>
			</a>
			<a href="water">
				<div class="arrow_left arrow_trading">
					<i class="fa fa-arrow-left" aria-hidden="true"></i>
					<strong>Previous</strong>

				</div>
			</a>

		</div>

		<div class="product_page_button trading_button">
			<a href="http://graeps.com/Emptor/trading/#section_5">
				<p>
					<strong>Request more information</strong>
				</p>
			</a>
		</div>

	</div>

</body>

</html>